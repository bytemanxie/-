...
<script setup>
	...
	const searchUserByAccount = async () => {
		tableData.value = (await searchUser(adminAccount.value,'用户')).data
	}
	// 部门数据
	...
	const searchForDepartment = async () => {
		...
		tableData.value = (await searchDepartment(department.value)).data
	...
	const currentChange = async (value) => {
		...
		tableData.value = (await returnListData(value, '用户')).data
	}


	// 筛选冻结用户
	const banUserList = async () => {
		tableData.value = (await getBanList()).data
	}
